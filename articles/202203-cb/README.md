...
1. **約30秒** ビルドが早く始まるようになりました
2. Cloud Build が Debian で動くようになりました(これまで Ubuntu)

※1. ただしマシンタイプを指定しないビルド(`E2_MEDIUM` 利用時)は除きます。`E2_MEDIUM`は元から高速(**7秒**)にビルドが始まります。
...
すでに昔の GCE にはアクセスできないため、比較対象のデータは **技術書典12の同人誌で使ったデータ(2021/12/20頃に取得)** とします。(一番右の数字がプロビジョニング時間で `E2_MEDIUM` 以外のマシンタイプでは **70秒超** かかっています)
